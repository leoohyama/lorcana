...
master_target_cards <- read_csv("data/target_cards_with_epids2.csv", 
...
token_res <- POST(
  ...
  add_headers("Authorization" = paste("Basic", auth_string), "Content-Type" = "application/x-www-form-urlencoded"),
...
get_ebay_active_listings <- function(card_name, version, rarity, token, coll_num, epid_code = NA_character_) {
  ...
  template <- tibble(
    ...
    item_url = character(), is_graded = logical(), seller_name = character(), 
    feedback_pct = numeric(), feedback_num = numeric(), pull_source = character(), 
  ...
  fetch_and_parse <- function(query_params, source_name) {
    ...
    res <- GET("https://api.ebay.com/buy/browse/v1/item_summary/search",
               query = query_params,
  ...
  epid_results <- if (!is.na(epid_code) && epid_code != "") fetch_and_parse(list(epid = epid_code, limit = 200), "EPID") else template
  
  combined_unique <- bind_rows(text_results, epid_results) %>% distinct(item_id, .keep_all = TRUE)
  ...
  if (nrow(combined_unique) > 0) {
    ...
    version_keys <- raw_v_keys[nchar(raw_v_keys) > 3] # Naturally drops words 3 letters or fewer

    # 2. SLIMMED BLACKLIST
    blacklist <- "case|box|proxy|replica|repro|custom|fan art|digital|wafer"

    # 3. VECTORIZED FILTER LOGIC (Fixes the map_lgl crash)
    final_df <- combined_unique %>% 
      mutate(lower_title = tolower(listing_title)) %>%
      # Pre-calculate the flags for every row cleanly
      mutate(
        ...
        has_num    = if(!is.na(coll_num) && coll_num != "") str_detect(lower_title, paste0("\\b", coll_num, "\\b")) else FALSE,
        has_ver    = if(length(version_keys) > 0) map_lgl(lower_title, ~ any(str_detect(.x, fixed(version_keys)))) else FALSE
      ) %>%
      filter(
        pass_name,                                  # GATE 1: Name must match
        (pull_source == "EPID") | (has_rarity & (has_num | has_ver)), # GATE 2: Trust EPID OR check Rarity + Version/Num
        !str_detect(lower_title, blacklist),        # GATE 3: The New Slim Blacklist
        !is.na(price_val) & price_val >= 20.00      # GATE 4: The Relaxed $20 Floor
      ) %>%
      # Drop all the temporary logic columns so we only keep the clean data
...
if (nrow(final_gold_scrape) > 0) {
  ...
  # 2. Add the new data to the existing table
