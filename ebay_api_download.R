...
ui <- page_navbar(
  ...
  tags$head(
    tags$style(HTML("
      ...
      .flip-card-back { background-color: #2b3e50; color: white; transform: rotateY(180deg); border: 2px solid #18bc9c; display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 15px; text-align: center; }
      
      .badge-custom { position: absolute; top: -10px; right: -25px; background-color: #dc3545; color: white; border-radius: 12px; padding: 5px 12px; font-weight: bold; font-size: 14px; box-shadow: 0 2px 5px rgba(0,0,0,0.8); border: 2px solid #222; white-space: nowrap; z-index: 20; }
      .badge-rank { position: absolute; top: -10px; left: -15px; background-color: #f39c12; color: white; border-radius: 50%; width: 36px; height: 36px; display: flex; justify-content: center; align-items: center; font-weight: bold; font-size: 16px; box-shadow: 0 2px 5px rgba(0,0,0,0.8); border: 2px solid #222; z-index: 20; }
      
      .staleness-box { background-color: #2b3e50; border-left: 5px solid #18bc9c; padding: 15px; border-radius: 5px; margin-bottom: 15px; }
      
      .scrolling-wrapper { height: 750px; overflow-y: auto; overflow-x: hidden; position: relative; }
      ...
      .momentum-box { background: linear-gradient(135deg, #2b3e50, #1a252f); border-left: 5px solid #f39c12; padding: 15px; border-radius: 8px; margin-bottom: 10px; color: #ecf0f1; font-size: 15px;}
  ...
  nav_panel(title = "Market Overview", value = "Market Overview",
    layout_columns(
      ...
      card(
        ...
        # Outputting the combined text and image gallery
        uiOutput("momentum_statement"),
        # Shrunk plot to 450px to make room for the new 4-card gallery above it
...
server <- function(input, output, session) {
  ...
  # --- MASSIVE UPDATE: The Movers & Shakers Calculation & Gallery ---
  output$momentum_statement <- renderUI({
    ...
    momentum <- latest_prices %>%
      ...
      left_join(master_dict, by = "cardname") # Join dictionary to get the image folders/ids
    ...
    text_block <- tags$div(
      class = "momentum-box",
      tags$div(tags$strong("7-Day Relative Movers (%): "), "The biggest jump was ", HTML(pct_gainer_txt), ", while ", HTML(pct_loser_txt), " saw the steepest drop."),
      tags$div(style = "margin-top: 8px;", tags$strong("7-Day Absolute Movers ($): "), "The highest value gained was ", HTML(abs_gainer_txt), ", and the most value lost was ", HTML(abs_loser_txt), ".")
    )
